...
public class MdoRefBuilder {
  ...
  /**
   * Вычисляет ссылку для документа (файла).
   * Если документ имеет связь с MDClasses, то используется представление ссылки связанного объекта метаданных
   * иначе - используется строковое представление URI
   *
  ...
  /**
   * Формирует ссылку на объект-владелец метода
   *
   * @param documentContext Документ (модуль)
   * @param ctx             Узел вызова метода
   * @return Ссылка на объект-владелец
   */
  public String getMdoRef(DocumentContext documentContext, BSLParser.CallStatementContext ctx) {
  ...
  public String getMdoRef(
    DocumentContext documentContext, // todo переделать при переходе на проект
    @Nullable
